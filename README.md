...
Please note that the tutorials in this repository are subject to change. The current notebooks were developed with the intent to mostly use them inside ESA Datalabs, therefore the filepaths point to the Euclid Q1 data volume mounted in ESA Datalabs. It is possible to modify the notebooks to run outside of the platform if needed, but there are a few things to consider:
